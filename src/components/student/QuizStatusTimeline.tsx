...
const QuizItem = ({ quiz, studentName, handleStartQuiz }: { quiz: QuizTimelineItem, studentName: string, handleStartQuiz: (quiz: any) => void }) => {
  ...
  const attemptsCount = useMemo(() => quizAttempts.filter(a => a.quizId === quiz.id && a.studentName === studentName).length, [quizAttempts, quiz.id, studentName]);
  ...
  return (
    <li className={cn("flex flex-col lg:flex-row justify-between items-start lg:items-center p-4 border rounded-xl bg-white shadow-sm transition-all", quiz.status === 'Live' && "border-green-500 ring-2 ring-green-50")}>
      <div className="flex-1 space-y-1 mb-3 lg:mb-0">
        ...
        <div className="flex items-center gap-4 text-[11px] text-slate-400 pt-1">
          ...
          <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {quiz.startTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - {quiz.endTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
        </div>
      </div>
...
const QuizStatusTimeline = ({ studentName, quizzes: propQuizzes }: QuizStatusTimelineProps) => {
  ...
  const grouped = {
    ...
    Completed: processedQuizzes.filter(q => q.status === 'Completed'),
